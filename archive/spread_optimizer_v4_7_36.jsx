...
const SIMULATED_OAUTH_PROVIDERS = [
  { id: 'google',    name: 'Google',    color: '#fff', text: '#3c4043', glyph: 'G', border: 'border-stone-400' },
  { id: 'microsoft', name: 'Microsoft', color: '#fff', text: '#5e5e5e', glyph: '⊞', border: 'border-stone-400' },
  { id: 'x',         name: 'X',         color: '#000', text: '#fff',    glyph: '𝕏', border: 'border-stone-700' },
  { id: 'facebook',  name: 'Facebook',  color: '#1877f2', text: '#fff', glyph: 'f', border: 'border-blue-700' },
  { id: 'instagram', name: 'Instagram', color: '#e1306c', text: '#fff', glyph: '◷', border: 'border-pink-700' },
  { id: 'linkedin',  name: 'LinkedIn',  color: '#0a66c2', text: '#fff', glyph: 'in', border: 'border-blue-800' },
];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-sm p-4">
      <div className="w-full max-w-md bg-[#13131c] border border-stone-800/80 rounded-2xl p-6 shadow-2xl">
        <div className="text-center mb-5">
          ...
          <div className="text-[10px] text-stone-500 font-num tracking-widest uppercase mt-1">
              ...
              {SIMULATED_OAUTH_PROVIDERS.map(p => (
                <Tooltip key={p.id} content={`Sign in with ${p.name} (simulated)`}>
                  <button
                    ...
                    className={`h-11 rounded-md border ${p.border} flex items-center justify-center text-base font-bold transition-opacity hover:opacity-80 disabled:opacity-50 disabled:cursor-wait`}
...
export default function SpreadOptimizer() {
  ...
  // v4.7.35: when the user re-detects bias AFTER an initial chain is loaded,
  // the cached chain is now stale relative to the new bias. We collapse the
  // candidate + Lookup sections until they explicitly Refresh chain & re-rank.
  // Tracked as a counter so we know "is this the FIRST detect of the session".
  const [biasDetectCount, setBiasDetectCount] = useState(0);
...
OUTPUT JSON only:
{
  ...
  "signals": {
    ...
    "spot_vs_wall": "<one sentence about where spot sits relative to wall>"
      ...
      // v4.7.35: if there's an existing chain AND this isn't the first bias
      // detect of the session, mark the chain as stale. The candidates +
      // Lookup grids collapse with a banner until Refresh chain & re-rank.
      setBiasDetectCount(n => {
        const next = n + 1;
        if (next > 1 && chainCache?.key === cacheKey) setChainStale(true);
        return next;
      });
  ...
  const fetchChain = useCallback(async (force = false) => {
    ...
    try {
      ...
      // v4.7.35: chain just refreshed — clear the bias-newer-than-chain flag
      // so the candidate + Lookup sections re-expand.
      setChainStale(false);
  ...
  const optimize = async () => {
    ...
    // read, click "Re-detect" — the chain auto-collapses
