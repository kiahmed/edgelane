...
function _buildTradierOrderBody(candidate, symbol, expiration, priceMode, limitPrice, opts) {
  ...
  const tag = (opts && opts.tag) || `edgelane_${candidate.label || 'candidate'}`.replace(/[^a-zA-Z0-9_]/g, '_').slice(0, 30);
  ...
  candidate.legs.forEach((leg, idx) => {
    ...
    const occ = _buildOccSymbol(symbol, expiration, leg.side, leg.strike);
    params.set(`option_symbol[${idx}]`, occ);
